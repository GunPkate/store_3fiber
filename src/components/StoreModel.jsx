...
import { FLOOR_W, FLOOR_D, OBS3D, POS_OBSTACLE, ATM_OBSTACLE, STOCK_OBSTACLE, BREAK_OBSTACLE } from '../engine/storeData.js';
...
function Walls() {
  return (
    <group>
      {/* visual-only thin reference walls (no collision) */}
...
/** Register screen with a slow color pulse, like the original. */
function RegisterScreen({ posO }) {
  const matRef = useRef();
  const t = useRef(0);
  useFrame((_, dt) => {
    t.current += dt;
  ...
  return (
    <group>
      <Box w={posO.hw * 2} h={1.0} d={posO.hd * 2} color={0x5c3d1e} x={posO.x} y={0.5} z={posO.z} />
      <Box w={posO.hw * 2} h={0.06} d={posO.hd * 2} color={0x222222} x={posO.x} y={1.02} z={posO.z} />
      <Box w={0.5} h={0.4} d={0.35} color={0x111111} x={-1.2} y={1.24} z={posO.z} />
      <Box w={0.5} h={0.25} d={0.04} color={0x111111} x={-1.2} y={1.52} z={posO.z - 0.36} />
      <mesh position={[-1.2, 1.52, posO.z - 0.34]}>
...
function Atm({ atmO }) {
  return (
    <group>
      <Box w={atmO.hw * 2} h={1.8} d={atmO.hd * 2} color={0x222222} x={atmO.x} y={0.9} z={atmO.z} />
      <mesh position={[atmO.x, 1.2, atmO.z - atmO.hd - 0.02]}>
...
/** Floating stock-level bar above each shelf — DOM widget anchored in 3D space. */
function StockBars({ items }) {
  // engine.items mutate in place; subscribe to the throttled HUD tick so
  // this component re-renders periodically and picks up the latest qty.
  useUiStore((s) => s.hud);
  const shelfObs = OBS3D.filter((o) => o.label.startsWith('Shelf'));
  return (
    <>
      {items.map((si, i) => {
        const o = shelfObs[i];
...
/**
 * The whole static store. `onFloorClick` receives a {x,z} world point for
 * tool interactions (add waypoint / spawn npc / fallback near-click picks).
 */
export default function StoreModel({ onFloorClick }) {
  const shelfObs = OBS3D.filter((o) => o.label.startsWith('Shelf'));
  const fridgeObs = OBS3D.filter((o) => o.label.startsWith('Fridge'));
  ...
  return (
    <group>
      ...
      <RegisterScreen posO={POS_OBSTACLE} />
      <Atm atmO={ATM_OBSTACLE} />
      <Box
